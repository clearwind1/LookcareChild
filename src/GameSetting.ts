...
class GameSetting extends Othercontainer
{
    ...
    protected show()
    {
        ...
        for (var i: number = 0; i < 2; i++) {
            ...
            var onposx: number = 522;
            var offposx: number = 400;
            var posx: number = this.soundswitch[i] ? onposx : offposx;
            GameUtil.relativepos(switchonoff, setttingbg, posx, 132 + i * 122);
            ...
            GameUtil.relativepos(this.switchbtn[i], setttingbg, 460, 132 + i * 122);
        }

        var close: GameUtil.Menu = new GameUtil.Menu(this, 'closebtn_png', 'closebtn_png', this.close);
        this.addChild(close);
        GameUtil.relativepos(close, setttingbg, 596, 27);
    }
